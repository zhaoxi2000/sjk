...
$(document).ready(function() {
    ...
    var tagType = queryString('tagType');
    ...
    if(tagType!=0){   	
    	$('#keyTagType option[value="'+tagType+'"]').attr('selected','selected');
    }
    LoadTags(tagType||1,tagId);
    loadGridTable(tagId);
...
function loadGridTable(tagId) {
    ...
    if(tagId != 0) {
        url = urlWithParam + "/admin/mo-tagapp/search.json?tagId=" + tagId;
    }
    $('#dataList').datagrid({
        loadMsg : '正在加载...',
        iconCls : 'icon-save',
...
function loadsltTagId(pid, value) {
    ...
    $.ajax({
        ...
        success : function(data) {
            if(data != null) {
                var data = data.rows;
                getSelectData(data, $TagId, value);
            }
        }
    });
}

function LoadTags(pid,tagId) {
    $keyTagId = $("#keyTagId");
    if(pid == 1 || pid == "1") {
        url = urlWithParam + "/admin/motag/topic-list.json"
    } else {
    ...
    $.ajax({
        ...
        success : function(data) {
            if(data != null) {
                var data = data.rows;
                getSelectData(data, $keyTagId, tagId);
            }
            $keyTagId.bind({
                change : function() {
                    SearchEve()
