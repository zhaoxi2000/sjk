...
function loadGridTable() {
    $('#softList').datagrid({
        ...
        striped : true,
        collapsible : false,
        url : urlWithParam + "/admin/marketapp/search.json?catalog=100",
        pageSize : 20,
        sortOrder : 'desc',
        idField : "id",
        ...
        columns : [[{
            ...
            formatter : function(value, rec, index) {
                var ids = '<img class="w_20 h_20 f_l mt_4 mr_10" src="' + rec.logoUrl + '"/><a name="softIds" class="co_bl f_l" value="' + rec.id + '" href="../admin/marketapp/' + rec.id + '.edit.d" target="_blank">' + value + '</a>';
                return ids;
            }
        }, {
            field : 'version',
            ...
            width : 180,
            formatter : function(value, rec, index) {
                var edit = '<a name="softIds" class="co_bl f_l mr_15" value="' + rec.id + '" href="../admin/marketapp/' + rec.id + '.edit.d' + '"  target="_blank">编辑</a>';
...
function SearchEve() {
    ...
    var cputype = $.trim($('#keyCpu option:selected').val());
...
function showEve() {
    ...
    $.messager.confirm('确认', '您确认要显示这条记录吗?', function(row) {
        if(row) {
            $.ajax({
                type : "POST",
                url : urlWithParam + "/admin/marketapp/show.d",
                traditional : true,
                data : {
                    "ids" : softId
                },
...
function hideEve() {
    ...
    $.messager.confirm('确认', '您确认要隐藏这条记录吗?', function(row) {
        if(row) {
            $.ajax({
                type : "POST",
                url : urlWithParam + "/admin/marketapp/hide.d",
                traditional : true,
                data : {
                    "ids" : softId
                },
...
function UpIdsType() {
    ...
    $IdsMenu_li_a.click(function(e, index) {
        ...
        if(inputs !== "") {
            $.ajax({
                type : "POST",
                url : urlWithParam + "/admin/marketapp/edit-catalog.d",
                traditional : true,
                data : {
                    "catalog" : 100,
                    "subCatalog" : typeIds,
                    "subCatalogName" : typeName,
                    "ids" : types
                },
                dataType : 'json',
                success : function(data) {
...
// for ( var i = 0; i < len; i++) {
// tagwareArr.push([ "<li><a tagid='", tagsMap[i].id, "' tagname='",
// tagsMap[i].name, "' tagType='", tagsMap[i].tagType, "'>",
// tagsMap[i].name, "</a></li>" ].join(''));
// }
// var html = [];
...
// $TagMenu.find("a[tagid='" + data.data[t].tagId + "']").attr(
// "value", 1).addClass("cur")
// }
// UpTag(softId)
// }
// })
...
// var $TagMenu_li_a = $("#TagMenu li a"), $TagMenu = $("#TagMenu"), $editTag =
// $("#editTag"), $tagPost = $("#tagPost");
//
// $TagMenu_li_a.bind({
// click : function() {
...
// .bind({
// click : function() {
// var tagsLen = $TagMenu.find("a[value=1]").length, tagsid = "", tagsids = [],
// tagname = "", tagnames = [], tagType = "", tagTypes = [];
//
// for ( var id = 0; id < tagsLen; id++) {
// tagsid = parseInt($TagMenu.find("a[value=1]").eq(id)
// .attr("tagid"));
// tagsids[id] = tagsid;
//
// tagname = $TagMenu.find("a[value=1]").eq(id).attr(
// "tagname");
// tagnames[id] = tagname;
//
// tagType = $TagMenu.find("a[value=1]").eq(id).attr(
// "tagType");
// tagTypes[id] = tagType;
// }
// ;
...
// // 批量更新标签
// function UpIdsTag() {
// var $TagsMenu_li_a = $("#TagsMenu li a"), $TagsMenu = $("#TagsMenu"),
// $editTags = $("#editTags"), $tagsPost = $("#tagsPost");
//
// $TagsMenu_li_a.bind({
// click : function() {
// if (parseInt($(this).attr("value")) !== 1) {
...
// click : function() {
// var inputs = $(".datagrid-body tr[class*='datagrid-row-selected']").length,
// softId = "", softIds = [], tagsLen = $TagsMenu
// .find("a[value=1]").length, tagsid = "", tagsids = [], tagname = "", tagnames
// = [], tagType = "", tagTypes = [];
//
// for ( var i = 0; i < inputs; i++) {
// softId = parseInt($(
...
//
// for ( var id = 0; id < tagsLen; id++) {
// tagsid = parseInt($TagsMenu.find("a[value=1]").eq(id)
// .attr("tagid"));
// tagsids[id] = tagsid;
//
// tagname = $TagsMenu.find("a[value=1]").eq(id).attr(
// "tagname");
// tagnames[id] = tagname;
//
// tagType = $TagsMenu.find("a[value=1]").eq(id).attr(
// "tagType");
// tagTypes[id] = tagType;
// }
// ;
...
// for ( var i = 0; i < len; i++) {
// topicsArr.push([ "<li><a tagid='", topicsMap[i].id, "' tagname='",
// topicsMap[i].name, "' tagType='", topicsMap[i].tagType, "'>",
// topicsMap[i].name, "</a></li>" ].join(''));
// }
// var html = [];
...
//
// for ( var t = 0; t < lentag; t++) {
// $TopicsMenu.find("a[tagid='" + data.data[t].tagId + "']").attr(
// "value", 1).addClass("cur")
// }
// UpTopics(softId)
// }
...
// var $TopicsMenu_li_a = $("#TopicsMenu li a"), $TopicsMenu = $("#TopicsMenu"),
// $editTopics = $("#editTopics"), $TopicsPost = $("#TopicsPost");
//
...
// .bind({
// click : function() {
// var topicsLen = $TopicsMenu.find("a[value=1]").length, tagsid = "", tagsids =
// [], tagname = "", tagnames = [], tagType = "", tagTypes = [];
//
// for ( var id = 0; id < topicsLen; id++) {
// tagsid = parseInt($TopicsMenu.find("a[value=1]").eq(id)
// .attr("tagid"));
// tagsids[id] = tagsid;
// tagname = $TopicsMenu.find("a[value=1]").eq(id).attr(
// "tagname");
// tagnames[id] = tagname;
// tagType = $TopicsMenu.find("a[value=1]").eq(id).attr(
// "tagType");
// tagTypes[id] = tagType;
// }
// ;
...
// var $TopicssMenu_li_a = $("#TopicssMenu li a"), $TopicssMenu =
// $("#TopicssMenu"), $editTopicss = $("#editTopicss"), $TopicssPost =
// $("#TopicssPost");
//
...
// click : function() {
// var inputs = $(".datagrid-body tr[class*='datagrid-row-selected']").length,
// softId = "", softIds = [], tagsLen = $TopicssMenu
// .find("a[value=1]").length, tagsid = "", tagsids = [], tagname = "", tagnames
// = [], tagType = "", tagTypes = [];
//
// for ( var i = 0; i < inputs; i++) {
...
//
// for ( var id = 0; id < tagsLen; id++) {
// tagsid = parseInt($TopicssMenu.find("a[value=1]")
// .eq(id).attr("tagid"));
// tagsids[id] = tagsid;
// tagname = $TopicssMenu.find("a[value=1]").eq(id).attr(
// "tagname");
// tagnames[id] = tagname;
// tagType = $TopicssMenu.find("a[value=1]").eq(id).attr(
// "tagType");
// tagTypes[id] = tagType;
// }
// ;
...
// var $editDelta = $("#editDelta"), $addDelta = $("#addDelta"), $closeDelta =
// $("#closeDelta"), $deltaDownload = $("#deltaDownload"), deltaDownload = 0;
//
// $editDelta.window({
// title : '修改增减量',
// top : 200,
...
function del() {
    ...
    $.messager.confirm('确认', '您确认要彻底删除这条记录吗?', function(row) {
        if(row) {
            ...
            $.ajax({
                type : "POST",
                url : urlWithParam + "/admin/marketapp/del.d",
                traditional : true,
                dataType : 'json',
                data : {
                    "id" : ids
...
function editKeywords(keywords, id) {
    ...
    btn.unbind('click').bind('click', function() {
        $.ajax({
            url : '/sjk-market-admin/admin/marketapp/edit-keywords.d',
            type : 'POST',
            data : {
                "id" : id,
