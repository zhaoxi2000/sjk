...
},beforePageText:"页",afterPageText:"共<b> {pages}</b>",displayMsg:"当前显示<b> {from} - {to} </b>条，共 <b>{total}</b> 条记录"};
})(jQuery);
(function($){
function _92(_93){
var _94=$(_93);
...
}else if(opts.pagePosition=="both"){
	_457.addClass("datagrid-pager-top").prependTo(_452);
	var ptop=$("<div class=\"datagrid-pager datagrid-pager-top\"></div>").prependTo(_452);
	_457.removeClass("datagrid-pager-top").appendTo(_452);
	_457=_457.add(ptop);
...
if(opts.pagerFixedTop && (opts.pagePosition=="top" || opts.pagePosition=="both") &&  (!_578.data('_fixDiv'))){
	_578.data('_fixDiv',true);
	_fixDiv('.datagrid:first','.datagrid-pager-top:first','fixedMove');
}
//fixedTopPager by chentiansong@ijinshan.com
function _fixDiv(topId,conId,cls){
	if(arguments.length<3) return false;
	var width = $(conId).width(),height = $(conId).height(),addDiv = $('<div/>');
		$(conId).width(width);
		addDiv.height(height).hide().prependTo(topId);
	$(window).bind({
		scroll:function(){
			
			var scrTop = document.documentElement.scrollTop + document.body.scrollTop,			
				offTop = $(topId)[0].getBoundingClientRect().top + scrTop - document.documentElement.clientTop;
				if(scrTop > offTop){
					addDiv.show();
					$(conId).addClass(cls);	
				}
