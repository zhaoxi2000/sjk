    ...
    $('#softList').datagrid({
        ...
        columns : [[{
            ...
            formatter : function(value, rec, index) {
                ...
                //var tagapp = '<a class="mr_15 co_bl f_l" onclick="tagappEve(' + rec.id + ',' + index + ')">添加摇一摇</a>';
                if(rec.hidden==true){
...
//批量将其他市场应用转移到ijinshan市场
function BatchAppToijinshan(){ 
	var inputs = $(".datagrid-body tr[class*='datagrid-row-selected']").length;
    var type = "";
    var ids = [];
    for(var i = 0; i < inputs; i++) {
	...
	$.messager.confirm('确认', '您确认要隐藏这些记录吗?', function(row) {
        if(row) {
            $.ajax({
                type : "POST",
                url : urlWithParam + "/admin/ijinshan/shift-to-ijinshan.json",
                traditional : true,
                data : {
                    "ids" : ids
                },
...
function tagappEve() {
    var softId = arguments[0];
    $.messager.confirm('确认', '您确认要添加这条记录吗?', function(row) {
        if(row) {
            $.ajax({
                type : "POST",
