...
public class SearchService1Impl implements SearchService1 {
    ...
    public final Pager<MobileSearchApp> emptyResultPager = new Pager<MobileSearchApp>();
    ...
    @Override
    public Pager<MobileSearchApp> search(String q, int page, int rows, Integer noAds, Integer official)
            throws Exception {
        ...
        Pager<MobileSearchApp> pager = searchInTotalhitsByQuery(topDocs, page, rows);

        return pager;
    }

    @Override
    public Pager<MobileSearchApp> searchInTotalhitsByQuery(TopDocs topDocs, int page, int rows) throws Exception {
        final Pager<MobileSearchApp> pager = new Pager<MobileSearchApp>();
        if (topDocs == null) {
            pager.setRows(0);
            return pager;
        }
        ...
        if (hitCount > 0) {
            ...
            if (validSize > 0) {
                ...
                final List<MobileSearchApp> list = appService.getSearchAppByIds(mutileSearchIds);
